...
for l in range(0, 466):
    ...
    #Determin the best reachable node for each cluster
    i = 0
    for i in range(len(Cluster)):
        best_dist = "None"
...
#The option to use a weighting factor that applied to the distance measure is included.
    ...
    #Pick the cluster with node that is closest to a cluster centroid. Simple
    C_star = Best_Reach_Dist.index(min(Best_Reach_Dist))

#Selects cluster with fewest elements. Tiebreak follows same logic as weight balancing tiebreak.  
    """
